	...
	protected void startSection(Template template)
	{
		...
		if (multiSection)
		{
			...
			/*  TODO das scheint nicht zu funktionieren          outstream.print("\\headery");
						...
						outstream.println();*/
			if (template.getPage().getFirstPageFooter() != template.getPage().getMiddlePagesFooter() || template.getPage().getFirstPageHeader() != template.getPage().getFurtherPagesHeader())
			{// Other Header or Footers for the first page

...
public abstract class TextStyle extends StyleBase implements Serializable {
    ...
    public boolean textStyleEquals(TextStyle s) {
        ...
        if (s == null)
            return false;
        return eq(font, s.font) && eq(background, s.background) && eq(foreground, s.foreground)
                && eq(size, s.size) && eq(bold, s.bold) && eq(italic, s.italic) && eq(underlined, s.underlined)
                && eq(backgroundImageName, s.backgroundImageName) && FlexSize.eq(backgroundImageTargetWidth, s.backgroundImageTargetWidth)
                && eq(datePattern, s.datePattern) && eq(floatPattern, s.floatPattern) && eq(intPattern, s.intPattern) && eq(lineSpacingNumber, s.lineSpacingNumber);
    }
